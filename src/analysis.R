...
if (all(c("Cholesterol", "Heart.Rate") %in% names(data))) {
  ...
  ggsave(filename = file.path(out_dir, "cholesterol_vs_heart_rate_scatter.png"), plot = p4, width = 8, height = 6)
}
...
if (all(c("Cholesterol", "Heart.Rate") %in% names(data))) {
  ...
  ggsave(filename = file.path(out_dir, "cholesterol_vs_heart_rate_heatmap.png"), plot = p5, width = 8, height = 6)
}
...
if ("Continent" %in% names(data)) {
  ...
  ggsave(filename = file.path(out_dir, "age_group_continent_heatmap.png"), plot = p6, width = 10, height = 6)
}
